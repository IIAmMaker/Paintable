...
public class ModItemTagGenerator extends ItemTagsProvider {
    ...
    protected void addTags(HolderLookup.Provider pProvider) {
        ...
        for (DyeColor color : colors) {
            ...
            this.tag(PTags.Items.PAINTED_LOG_TAGS[color.getId()]).add(block.asItem());
        }
        for (DyeColor color : colors) {
        ...
        for (DyeColor color : colors) {
            ...
            this.tag(PTags.Items.FILLED_PAINT_BUCKET).add(block.asItem());
        }
        for (DyeColor color : colors) {
            Item item = itemN(color + "_paint_brush");
            this.tag(PTags.Items.DIPPED_PAINT_BRUSH).add(item);
        }
        for (DyeColor color : colors) {
