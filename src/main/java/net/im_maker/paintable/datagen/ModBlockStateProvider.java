...
public class ModBlockStateProvider extends BlockStateProvider {
    ...
    private void filledPaintBucketBlock(Block b, String texture) {
        ...
        for (int i = 0; i <= 4; i++) {
            ...
            ConfiguredModel eModel = ConfiguredModel.builder()
                    .modelFile(models().withExistingParent("paint_bucket/level" + num + "/" + name(b) + "_level" + num, new ResourceLocation(Paintable.MOD_ID, "block/template_paint_bucket" + p))
                            .texture("paint_bucket", paint_bucket).renderType("cutout"))
                    .buildLast();
    ...
    private void paintBucketBlock(Block b, String texture) {
        ResourceLocation paint_bucket = new ResourceLocation(Paintable.MOD_ID, "block/" + texture);
        ConfiguredModel eModel = ConfiguredModel.builder()
                .modelFile(models().withExistingParent("paint_bucket/" + name(b), new ResourceLocation(Paintable.MOD_ID, "block/template_paint_bucket"))
                        .texture("paint_bucket", paint_bucket).renderType("cutout"))
                .buildLast();
        ConfiguredModel nModel = ConfiguredModel.builder()
                .modelFile(models().withExistingParent("paint_bucket/" + name(b), new ResourceLocation(Paintable.MOD_ID, "block/template_paint_bucket"))
    ...
    protected void registerStatesAndModels() {
        ...
        for (DyeColor color : colors) {
            ...
            doorBlockWithRenderType(((DoorBlock) block(color, "door")), modLoc("block/" + color + "_painted_door_bottom"), modLoc("block/" + color + "_painted_door_top"), "cutout");
            trapdoorBlockWithRenderType(((TrapDoorBlock) block(color, "trapdoor")), modLoc("block/" + color + "_painted_trapdoor"), true, "cutout");
