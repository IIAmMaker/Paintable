...
public class ModBlockTagGenerator extends BlockTagsProvider {
    ...
    protected void addTags(HolderLookup.Provider pProvider) {
        ...
        for (DyeColor color : colors) {
            ...
            this.tag(ModBlockTags.PAINTED_LOG_TAGS[color.getId()]).add(block);
        }
        for (DyeColor color : colors) {
        ...
        for (DyeColor color : colors) {
            ...
            this.tag(ModBlockTags.PAINTABLE_BRICKS).add(bricks, Blocks.BRICKS);
            this.tag(BlockTags.STAIRS).add(brick_stairs);
            this.tag(ModBlockTags.PAINTABLE_BRICK_STAIRS).add(brick_stairs, Blocks.BRICK_STAIRS);
            this.tag(BlockTags.SLABS).add(brick_slab);
            this.tag(ModBlockTags.PAINTABLE_BRICK_SLABS).add(brick_slab, Blocks.BRICK_SLAB);
            this.tag(BlockTags.WALLS).add(brick_wall);
            this.tag(ModBlockTags.PAINTABLE_BRICK_WALLS).add(brick_wall, Blocks.BRICK_WALL);
        }
        for (DyeColor color : colors) {
            ...
            this.tag(ModBlockTags.PAINTABLE_MUD_BRICKS).add(mud_bricks, Blocks.MUD_BRICKS);
            this.tag(BlockTags.STAIRS).add(mud_brick_stairs);
            this.tag(ModBlockTags.PAINTABLE_MUD_BRICK_STAIRS).add(mud_brick_stairs, Blocks.MUD_BRICK_STAIRS);
            this.tag(BlockTags.SLABS).add(mud_brick_slab);
            this.tag(ModBlockTags.PAINTABLE_MUD_BRICK_SLABS).add(mud_brick_slab, Blocks.MUD_BRICK_SLAB);
            this.tag(BlockTags.WALLS).add(mud_brick_wall);
            this.tag(ModBlockTags.PAINTABLE_MUD_BRICK_WALLS).add(mud_brick_wall, Blocks.MUD_BRICK_WALL);
        }
        for (DyeColor color : colors) {
            Block concrete = Paintable.getBlockFromString("minecraft", color + "_concrete");
            this.tag(ModBlockTags.PAINTABLE_CONCRETE_VANILLA).add(concrete);
        }
        if (ModList.get().isLoaded("dye_depot")) {
            for (DDDyes color : colors2) {
                Block concrete_dye_depot = Paintable.getBlockFromString("dye_depot", color + "_concrete");
                this.tag(ModBlockTags.PAINTABLE_CONCRETE_DYE_DEPOT).add(concrete_dye_depot);
            }
            this.tag(ModBlockTags.PAINTABLE_CONCRETE).addTags(ModBlockTags.PAINTABLE_CONCRETE_VANILLA, ModBlockTags.PAINTABLE_CONCRETE_DYE_DEPOT);
        } else {
            this.tag(ModBlockTags.PAINTABLE_CONCRETE).addTag(ModBlockTags.PAINTABLE_CONCRETE_VANILLA);
        }
        for (DyeColor color : colors) {
            Block block = blockN(color + "_paint_bucket");
            this.tag(BlockTags.MINEABLE_WITH_PICKAXE).add(block);
            this.tag(ModBlockTags.FILLED_PAINT_BUCKET).add(block);
        }
        for (DyeColor color : colors) {
